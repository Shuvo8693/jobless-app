...
class LikeController extends GetxController {
  ...
  LikeController({required this.timelinePostController});
  ...
  Future<void> sendLike(dynamic postId) async {
    ...
    try {
      ...
      if (response.statusCode == 201 || response.statusCode == 200) {
        ...
        var postIndex = timelinePostController.timeLinePost.value.results?.indexWhere((post) => post.sId == postId); // find the post index
        print(postIndex.toString());

        if (postIndex != null && postIndex != -1) {
          var post = timelinePostController.timeLinePost.value.results?[postIndex];
