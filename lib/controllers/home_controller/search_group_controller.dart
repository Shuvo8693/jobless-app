...
class GroupSearchController extends GetxController {
  Rx<GroupSearchResultModel> groupSearchResultModel = GroupSearchResultModel().obs;
  ...
  searchGroup({bool isLoadMore = false, required String name}) async {
    ...
    try {
      String url = '${ApiConstants.groupSearchResultUrl(name)}&page=${currentPage.value}&limit=${limit.value}';
      ...
      if (response.statusCode == 200) {
        ...
        if (isLoadMore) {
          ...
          groupSearchResultModel.value.data?.attributes?.results?.addAll(results.map((result) => GroupSearchResult.fromJson(result)));
        } else {
          // Initial load
          groupSearchResultModel.value= GroupSearchResultModel.fromJson(responseData);
        }
